...
export type TApiServiceRequestMethods =
  | 'GET'
  | 'POST'
  | 'DELETE'
  | 'PATCH'
...
export type TApiServiceRequestParams<
  B extends TApiServiceBody = TApiServiceBody,
> = {
...
export type TApiTransportConnector = <R, B extends TApiServiceBody>(
  arg0: TApiServiceRequestParams<B>,
) => Promise<TApiServiceRequestDescriptor<R>>;

export type TApiTransport = {
  connect: TApiTransportConnector;
  abort(arg0: symbol): void;
};

export type TApiServiceRegistry = {
  requestInProgress: (arg0: TApiServiceRequestParams) => void;
  requestSuccessed: (
    arg0: TApiServiceRequestParams,
    arg1: TApiServiceResponse,
  ) => void;
...
export type THttpTransportParams = Partial<{
  baseURL: string;
  headers: Record<string, string>;

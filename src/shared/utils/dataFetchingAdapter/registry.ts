...
import { TStateActions } from 'shared/utils/types'; // eslint-disable-line
...
export class DataFetchingRegistry<S> {
  ...
  public registerSelector<D>(
    name: string,
    selector: TDataFetchingSelector<S, D>,
  ) {
    this._selectors[name] = selector;
  }

  public registerActions<D>(
    name: string,
    actions: TDataFetchingStateActions<D>,
  ) {

...
export const useRegistry = <R extends TRegistryType,>(): IRegistry<R> => {
  ...
  const add = useCallback((eventName: TRegistryEvent, eventHandler: TRegistryHandler) => {
  ...
  const remove = useCallback((eventName: TRegistryEvent, handlerKey: symbol) => {
...
export type TRegistrySubscriber<R extends TRegistryType> =(arg0: keyof R, arg1: R[keyof R]) => () => void;

export const useRegistrySubscriber = <R extends TRegistryType,>(registry: IRegistry<R>) => {
  return useCallback<TRegistrySubscriber<R>>((eventName, eventHandler) => {
...
export type TRegistryEmitter<R extends TRegistryType> = (arg0: keyof R, ...args: Parameters<R[keyof R]>) => void;

export const useRegistryEmitter = <R extends TRegistryType,>(registry: IRegistry<R>) => {
  return useCallback<TRegistryEmitter<R>>((eventName, ...args) => {
